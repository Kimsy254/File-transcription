...
def get_transcript_detail_form(request):
    ...
    if request.method == 'POST':
        ...
        def get_total_price():
            ...
            time_length_choice = request.POST.getlist('time_length', None)[0]
            time_length_price = {'Length': 1.00}.get(
                time_length_choice)
            multiplier += time_length_price

            text_format_choice = request.POST.getlist('text_format', None)[0]
            text_format_price = {'Verbatim': 0.15, 'CleanVerbatim': 0}.get(text_format_choice)
            multiplier += text_format_price

            num_speakers_choice = request.POST.getlist('number_of_speakers', None)[0]
            ...
            tat_choice = request.POST.getlist('turn_around_time', None)[0]
...
@login_required(login_url='/login/')
def save_to_cart(request, detail_id):
    TranscriptDetails.objects.filter(id=detail_id).update(saved=True)
    messages.success(request, 'Order Number %s has been added to Cart' % detail_id)
...
def get_my_cart(request):
    ...
    return render(request, "my-cart.html", args)
